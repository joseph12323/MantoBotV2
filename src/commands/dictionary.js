...
async function dictionary(matches, event, api, extra) {
    const { threadID, messageID } = event;
    const fs = require('fs');
    var text = matches[1]
    console.log(matches)
    if (matches[1]) {
      return require('axios').get(encodeURI(`https://api.dictionaryapi.dev/api/v2/entries/en/${text.trim().toLowerCase()}`)).then(res => {
        let data = res.data[0];
